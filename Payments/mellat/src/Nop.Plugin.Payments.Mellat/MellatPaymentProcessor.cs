...
namespace Nop.Plugin.Payments.Mellat
{
    public class MellatPaymentProcessor : BasePlugin, IPaymentMethod
    {
        ...
        {
            var orderId = GenerateRandomNumber(postProcessPaymentRequest.Order.Id);
            ...
            Bpm.Mellat.PaymentGatewayImplService bankService = new Bpm.Mellat.PaymentGatewayImplService();
            var result = bankService.bpPayRequest(long.Parse(_mellatPaymentSettings.TerminalId), _mellatPaymentSettings.Username, _mellatPaymentSettings.Password, orderId, long.Parse(postProcessPaymentRequest.Order.OrderTotal.ToString("#")), payDate, payTime, orderId.ToString(), callbackUrl, 0);

            return result;
        }

        protected int GenerateRandomNumber(int orderId)
        {
            Random rand = new Random();
            var seconds = DateTime.Now.Second;

            return rand.Next(1, int.MaxValue - orderId - seconds - 1) + orderId + seconds;
        }

        protected bool ValidatePayResponse(string result)
        {
            var splits = result.Split(',');
            if (splits.Count() != 2 || splits[0] != "0")
            {
                return false;
            }
            else
